...
object Preferences {
    ...
    private const val REPLAY_GAIN_ENABLED = "replay_gain_enabled"
    ...
    @JvmStatic
    fun isSynchronizationEnabled(): Boolean = isSyncronizationEnabled()
    ...
    fun isReplayGainEnabled(): Boolean {
        return App.getInstance().preferences.getBoolean(REPLAY_GAIN_ENABLED, true)
    }

    @JvmStatic
    fun setReplayGainEnabled(enabled: Boolean) {
        App.getInstance().preferences.edit().putBoolean(REPLAY_GAIN_ENABLED, enabled).apply()
    }

    @JvmStatic
    fun getReplayGainMode(): String? {
        return App.getInstance().preferences.getString(REPLAY_GAIN_MODE, "album")
    }

    @JvmStatic
    fun setReplayGainMode(mode: String) {
        App.getInstance().preferences.edit().putString(REPLAY_GAIN_MODE, mode).apply()
    }
    ...
    @JvmStatic
    fun showShibaMusicUpdateDialog(): Boolean {
        return App.getInstance().preferences.getLong(
                NEXT_UPDATE_CHECK, 0
        ) + 86400000 < System.currentTimeMillis()
    }
    ...
    @JvmStatic
    fun setShibaMusicUpdateReminder() {
