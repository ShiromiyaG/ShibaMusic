...
import com.shirou.shibamusic.util.Preferences.getImageSize
    ...
    private fun getStreamUri(): Uri {
        return when (type) {
            Constants.MEDIA_TYPE_MUSIC ->
                id?.let { MusicUtil.getStreamUri(it) } ?: Uri.EMPTY

            Constants.MEDIA_TYPE_PODCAST ->
                streamId?.let { MusicUtil.getStreamUri(it) } ?: Uri.EMPTY

            Constants.MEDIA_TYPE_RADIO ->
                streamUrl?.let(Uri::parse) ?: Uri.EMPTY

            else -> id?.let { MusicUtil.getStreamUri(it) } ?: Uri.EMPTY
        }
    }
}
