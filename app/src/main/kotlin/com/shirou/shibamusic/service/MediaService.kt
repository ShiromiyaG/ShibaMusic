...
class MediaService : MediaLibraryService() {
    ...
    private inner class CustomMediaLibrarySessionCallback : MediaLibrarySession.Callback {
        ...
        override fun onPlaybackResumption(
            ...
            if (media.isEmpty()) {
                Log.w(TAG, "onPlaybackResumption: queue vazia")
                return Futures.immediateFuture(
                    MediaSession.MediaItemsWithStartPosition(emptyList(), 0, C.TIME_UNSET)
                )
            }

            val filteredMedia = media.filterNotNull()
            if (filteredMedia.isEmpty()) {
                Log.w(TAG, "onPlaybackResumption: itens nulos removidos, lista vazia")
                return Futures.immediateFuture(
                    MediaSession.MediaItemsWithStartPosition(emptyList(), 0, C.TIME_UNSET)
                )
            }
            val mediaItems = MappingUtil.mapMediaItems(filteredMedia)
            if (mediaItems.isEmpty()) {
                Log.w(TAG, "onPlaybackResumption: mapMediaItems retornou vazio")
                return Futures.immediateFuture(
                    MediaSession.MediaItemsWithStartPosition(emptyList(), 0, C.TIME_UNSET)
                )
            }

            val playableItems = mediaItems.filter { mediaItem ->
                if (mediaItem.localConfiguration == null) {
                    Log.e(TAG, "onPlaybackResumption: MediaItem sem localConfiguration, id=${mediaItem.mediaId}")
                    false
                } else {
                    true
                }
            ...
            if (playableItems.isEmpty()) {
                Log.e(TAG, "onPlaybackResumption: nenhum MediaItem com localConfiguration válido")
                return Futures.immediateFuture(
                    MediaSession.MediaItemsWithStartPosition(emptyList(), 0, C.TIME_UNSET)
                )
            }

            val startIndex = queueRepository.getLastPlayedMediaIndex().coerceIn(0, playableItems.lastIndex)
            val startPosition = queueRepository.getLastPlayedMediaTimestamp().coerceAtLeast(0)

            Log.d(
                TAG,
                "onPlaybackResumption: itens=${playableItems.size}, startIndex=$startIndex, startPosition=$startPosition, primeiraUri=${playableItems[startIndex].localConfiguration?.uri}"
            )

            customLayout = buildCustomLayout(player)
            session.setCustomLayout(customLayout)

            return Futures.immediateFuture(
                MediaSession.MediaItemsWithStartPosition(playableItems, startIndex, startPosition)
            )
        }
